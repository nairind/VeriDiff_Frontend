...
export default function SignIn() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result?.ok) {
        // Check for redirect parameter, fallback to home page
  ...
  // Preserve redirect parameter in sign-up link
  const signUpUrl = router.query.redirect 
    ? `/auth/signup?redirect=${encodeURIComponent(router.query.redirect)}`
    : '/auth/signup'
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', color: '#6b7280' }}>
          ...
          <Link href={signUpUrl} style={{ color: '#2563eb', textDecoration: 'none', fontWeight: '500' }}>
            Sign Up
          </Link>
        </div>
      </div>
