...
export default function Home() {
  ...
  const handleFilesReady = async ({ file1, file2 }) => {
    try {
      // Store files in sessionStorage for the comparison pages
      sessionStorage.setItem('file1', JSON.stringify({
        name: file1.name,
        size: file1.size,
        type: file1.type,
        lastModified: file1.lastModified
      }));
      sessionStorage.setItem('file2', JSON.stringify({
        name: file2.name,
        size: file2.size,
        type: file2.type,
      ...
      const file1ArrayBuffer = await file1.arrayBuffer();
      const file2ArrayBuffer = await file2.arrayBuffer();
      
      sessionStorage.setItem('file1Data', btoa(String.fromCharCode(...new Uint8Array(file1ArrayBuffer))));
      sessionStorage.setItem('file2Data', btoa(String.fromCharCode(...new Uint8Array(file2ArrayBuffer))));
